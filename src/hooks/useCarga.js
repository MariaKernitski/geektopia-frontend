...
export function useCarga(buscar) {
  const [estado, setEstado] = useState({ dados: null, erro: '', carregado: false });
  ...
  useEffect(() => {
    ...
    buscar()
      .then((dados) => { if (ativo) setEstado({ dados, erro: '', carregado: true }); })
      .catch((err) => {
        if (ativo) setEstado({ dados: null, erro: mensagemDeErro(err, 'Não foi possível carregar os dados.'), carregado: false });
      });
